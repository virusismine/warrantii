...
//  print_r(Input::get('id1'));exit();
        $rules = array();
        $validator = Validator::make($data = Input::all(), $rules);

//        print_r(count(Input::get('id')));exit();
        if ($validator->fails()) {
            return Redirect::back()->withErrors($validator)->withInput();
        } else {
            DB::beginTransaction();
...
//            $user = Musers:: where('id', '=',\Session::get('uid'))->get();
            ...
            $s_quote = Organisation::where('ORG_UA_ID', '=', Input::get('PRINCIPAL_ID'))->get();
            $s_quote1 = Orgusers::where('ORG_UA_ID', '=', Input::get('PRINCIPAL_ID'))->get();

            Organisation:: where('ORG_UA_ID', '=', Input::get('PRINCIPAL_ID'))
                    ->update(array(
            ...
            Principal:: where('PRINCIPAL_ID', '=', Input::get('PRINCIPAL_ID'))
                    ->update(array(
                        'PRINCIPAL_CODE' => Input::get('ORG_NAME'),
                        'PRINCIPAL_NAME' => Input::get('ORG_CODE'),
                        'ULUO' => \Session::get('uid'),
            ));

            Orgusers:: where('ORG_UA_ID', '=', Input::get('PRINCIPAL_ID'))->where('UA_ID', '=', $s_quote1[0]->UA_ID)
                    ->update(array(
            ...
            Musers:: where('id', '=', $s_quote1[0]->UA_ID)
                    ->update(array(
                        ...
                        'addr_country' => Input::get('COUNTRY_CODE'),
                        'ULUO' => \Session::get('uid'),
            ));
            ...
            for ($i = 0; $i < count($idcall); $i++) {

                DB::table('principal_brand')->insert([
                    'BRAND_CODE' => $idcall[$i],
                    'PRINCIPAL_ID' => Input::get('PRINCIPAL_ID'),
                    'UCO' => \Session::get('uid'),
                ]);
            }

            
            $iddelete = Input::get('id1');
            
            for ($i = 0; $i < count($iddelete); $i++) { 
                
//                print_r(Input::get('PRINCIPAL_ID'));exit();
                
                
                    DB::table('principal_brand')->where('PRINCIPAL_ID', '=', Input::get('PRINCIPAL_ID'))
                            ->where('BRAND_CODE', '=', $iddelete[$i])->delete();
            }
